...
uv run georeset-wiki-landcover-embed-clip-patches \
...
uv run georeset-wiki-landcover-run-clip-linear-probe-experiment \
  --splits-path "${CLIP_OUTPUT_DIR}/label_splits.csv" \
...
uv run georeset-wiki-landcover-run-clip-zero-shot-experiment \
  --splits-path "${CLIP_OUTPUT_DIR}/label_splits.csv" \
  --embeddings-path "${CLIP_OUTPUT_DIR}/clip_embeddings.npz" \
  --output-dir "${CLIP_OUTPUT_DIR}" \
  --model-name "${CLIP_MODEL_NAME}" \
