...
from georeset_wiki_landcover.vision.linear_probe import predict_linear_probe
...
def evaluate_controls(
    *,
    ...
    for embeddings_path in embeddings_paths:
        ...
        split_train_name = "train_for_eval_strict"
        universe = splits[(splits["split"] == split_train_name) & (splits["tier"] == "all")].copy()
        if universe.empty:
            universe = splits[
                (splits["split"] == "train")
                & (splits["tier"] == "all")
        ...
        for tier in HARD_TIERS:
            observed = splits[
                (splits["split"] == split_train_name) & (splits["tier"] == tier)
            ].copy()
            if observed.empty:
                observed = splits[
                    (splits["split"] == "train")
