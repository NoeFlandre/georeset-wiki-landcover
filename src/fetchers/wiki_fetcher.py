...
class WikiFetcher:
    ...
    def get_articles_in_bounds(self, min_lon: float, min_lat: float, max_lon: float, max_lat: float, polygon_filter=None, osm_polygon_filter=None) -> List[Dict]:
        """
                ...
                for article in tile_articles:
                    ...
                    if pageid is not None and pageid not in seen_ids:
                        ...
                        if self._in_bounds(article, min_lon, min_lat, max_lon, max_lat):
                            ...
                            in_corine = polygon_filter is None or polygon_filter(article["lon"], article["lat"])
                            in_osm = osm_polygon_filter is None or osm_polygon_filter(article["lon"], article["lat"])
